...
class SingleTaskDataset(Dataset):
    def __init__(self, 
        ...
        data, tokenizer = self.load(path, is_train, maxlen, task_def, bert_model, do_lower_case) #factor 제거
        self._data = data
    ...
    @staticmethod
    def load(path, is_train=True, maxlen=512, task_def=None, factor=1.0, bert_model='bert-base-uncased', do_lower_case=True):
...
class Collater:
    ...
    def collate_fn(self, batch):
        ...
        task_weight = task_def.task_weight ####
        batch = new_batch
    ...
    def _prepare_model_input(self, batch, data_type):
        ...
        if self.__if_pair__(data_type):
            ...
            premise_masks = torch.ByteTensor(batch_size, tok_len).fill_(1) ############################premize_len
